...
function createDispatcher({
  ...
  function dispatchHandleMessage(sessionKey, chatId, msg, bot) {
    ...
    handleMessage(sessionKey, chatId, msg, bot).catch((err) => {
      // Once-and-for-all: auto-fix hexCorrupt errors without user intervention.
      // Sanitize session JSONL → kill stuck process → auto-retry turn.
      const errKind = classifyError(err).kind;
      if (errKind === 'hexCorrupt') {
        logger.warn?.(`[${sessionKey}] hexCorrupt detected — auto-fixing session JSONL + retry`);
        logEvent('hexcorrupt-autofix', {
          chat_id: chatId, session_key: sessionKey, error: err.message?.slice(0, 200),
        });
        // 1. Sanitize session JSONL
        try {
          const fs = require('fs'), path = require('path'), os = require('os');
          const sid = pm.get(sessionKey)?.claudeSessionId;
          if (sid) {
            const cwd = config.chats[chatId]?.cwd || os.homedir();
            const jp = path.join(os.homedir(), '.claude', 'projects', cwd.replace(/\//g, '-'), `${sid}.jsonl`);
            const raw = fs.readFileSync(jp, 'utf8');
            const clean = raw.replace(/\\x(?![0-9a-fA-F]{2})/g, '');
            if (clean !== raw) { fs.writeFileSync(jp, clean, { mode: 0o600 }); }
          }
        } catch {}
        // 2. Kill stuck process
        pm.kill(sessionKey, 'hexcorrupt-autofix').catch(() => {});
        // 3. Auto-retry the message (once only — guard against infinite loop)
        if (msg._isAutofixRetry) {
          logger.error?.(`[${sessionKey}] hexCorrupt autofix retry also failed — giving up`);
        } else {
          msg._isAutofixRetry = true;
