...
function createSlashCommands({
  ...
  async function showCommandPicker(query, chatId, sendReply) {
    ...
    const header = query
      ...
      : '📋 All commands — tap to run or /command <keyword> to search:';
    ...
    if (tg && bot) {
      ...
      await sendReply(`${header}\n\n${lines.join('\n')}\n\nType /command <keyword> to search.`);
    }
    return true;
  }
  ...
  async function handleCommandCallback(data, sendReply) {
    ...
    if (action === 'run') {
      ...
      await sendReply('Type `/command <keyword>` to search, e.g. `/command diff`');
    }
  }
  ...
  const dispatchSlashCommand = async function(ctx) {
    ...
    if (botAllowsCommands && (text === '/command' || text.startsWith('/command '))) {
      const query = text === '/command' ? '' : text.slice(9).trim();
