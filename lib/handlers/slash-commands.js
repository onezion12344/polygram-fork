...
function createSlashCommands({
  ...
  // All available commands (for /command picker) — core + extended
  const ALL_COMMANDS = [
    { cmd: 'stop', desc: 'Stop current turn', cat: 'Session' },
    { cmd: 'cancel', desc: 'Cancel current turn', cat: 'Session' },
    { cmd: 'model', desc: 'Switch AI model (sonnet/opus/haiku)', cat: 'Session' },
    { cmd: 'effort', desc: 'Set reasoning effort', cat: 'Session' },
    { cmd: 'sessions', desc: 'Browse and resume past sessions', cat: 'Session' },
    { cmd: 'resume', desc: 'Resume a session by ID', cat: 'Session' },
    { cmd: 'new', desc: 'Start a new session', cat: 'Session' },
    { cmd: 'clear', desc: 'Clear conversation', cat: 'Session' },
    { cmd: 'compact', desc: 'Compact context', cat: 'Session' },
    { cmd: 'context', desc: 'Show context usage', cat: 'Session' },
    { cmd: 'cd', desc: 'Switch working directory', cat: 'Project' },
    { cmd: 'init', desc: 'Generate CLAUDE.md', cat: 'Project' },
    { cmd: 'diff', desc: 'Show git diff', cat: 'Project' },
    { cmd: 'review', desc: 'Code review changes', cat: 'Project' },
    { cmd: 'code_review', desc: 'Code review for bugs', cat: 'Project' },
    { cmd: 'add_dir', desc: 'Add working directory', cat: 'Project' },
    { cmd: 'doctor', desc: 'Environment diagnostics', cat: 'Project' },
    { cmd: 'memory', desc: 'Open memory files', cat: 'Project' },
    { cmd: 'agents', desc: 'Manage background subagents', cat: 'Agent' },
    { cmd: 'mcp', desc: 'Manage MCP servers', cat: 'Agent' },
    { cmd: 'plugin', desc: 'Manage plugins', cat: 'Agent' },
    { cmd: 'permissions', desc: 'Manage permission rules', cat: 'Agent' },
    { cmd: 'export', desc: 'Export conversation', cat: 'Session' },
    { cmd: 'rename', desc: 'Rename session', cat: 'Session' },
    { cmd: 'rewind', desc: 'Rewind to checkpoint', cat: 'Session' },
    { cmd: 'config', desc: 'View/change bot settings', cat: 'Session' },
    { cmd: 'status', desc: 'Show session status', cat: 'Session' },
    { cmd: 'stats', desc: 'Show usage stats', cat: 'Session' },
    { cmd: 'cost', desc: 'Show session cost', cat: 'Session' },
    { cmd: 'ide', desc: 'Connect to IDE', cat: 'Project' },
    { cmd: 'pr_comments', desc: 'Fetch PR comments', cat: 'Project' },
    { cmd: 'theme', desc: 'Change theme', cat: 'Session' },
  ];

  // /command [query] — search + category picker
  async function showCommandPicker(query, chatId, sendReply) {
    let results = ALL_COMMANDS;
    if (query) {
      const q = query.toLowerCase();
      results = ALL_COMMANDS.filter(c => c.cmd.includes(q) || c.desc.toLowerCase().includes(q) || c.cat.toLowerCase().includes(q));
    }

    if (results.length === 0) {
      await sendReply(`No commands matching "${query}".`);
      return true;
    }
    ...
    for (const [cat, cmds] of Object.entries(groups)) {
      // Category header
      keyboard.push([{ text: `📁 ${cat}`, callback_data: `cmd:cat:${cat}` }]);
      // 2 commands per row
      for (let i = 0; i < cmds.length; i += 2) {
        const row = [{ text: `/${cmds[i].cmd}`, callback_data: `cmd:run:${cmds[i].cmd}` }];
        if (cmds[i + 1]) row.push({ text: `/${cmds[i + 1].cmd}`, callback_data: `cmd:run:${cmds[i + 1].cmd}` });
        keyboard.push(row);
      }
    }

    // Search row
    keyboard.push([{ text: '🔍 Search', callback_data: `cmd:search:${chatId}` }]);

    const header = query
      ? `🔍 Commands matching "${query}":`
      : '📋 All commands — tap to run or /commandsearch <keyword> to search:';
    ...
    if (tg && bot) {
      ...
      }, { source: 'command-picker', botName });
    } else {
      const lines = results.slice(0, 15).map(c => `/${c.cmd} — ${c.desc}`);
      await sendReply(`${header}\n\n${lines.join('\n')}\n\nType /commandsearch <keyword> to search.`);
    }
    return true;
  }
  ...
  // Handle command picker callbacks (cmd:)
  async function handleCommandCallback(data, sendReply) {
    const parts = data.split(':');
    const action = parts[1];
    if (action === 'run') {
      const cmd = parts[2];
      await sendReply(`Run: /${cmd} (type it to execute)`);
    } else if (action === 'search') {
      await sendReply('Type `/commandsearch <keyword>` to search, e.g. `/commandsearch diff`');
    }
  }
  ...
  const dispatchSlashCommand = async function(ctx) {
    ...
    if (botAllowsCommands && text.startsWith('/model ')) {
      ...
      if (newModel && newModel.length > 0) {
        ...
        // Kill warm session so it respawns with the new model.
        // SDK's setModel() is unreliable for custom providers.
        await pm.kill(sessionKey, 'model-change').catch(() => {});
        await sendReply(`Model → ${newModel} (session will respawn on next message)`);
      } else {
        await sendReply(`Usage: /model <name>. Examples: opus, sonnet, haiku, deepseek-v4-pro`);
      }
      return true;
    }
    ...
    // /command [query] — search + browse all commands and skills
    if (botAllowsCommands && (text === '/commandsearch' || text.startsWith('/command '))) {
      const query = text === '/commandsearch' ? '' : text.slice(9).trim();
      return await showCommandPicker(query, chatId, sendReply);
    }
  ...
  dispatchSlashCommand.handleCommandCallback = handleCommandCallback;
