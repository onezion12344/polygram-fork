...
function createSlashCommands({
  ...
  return async function dispatchSlashCommand(ctx) {
    ...
    if (botAllowsCommands && text === '/sessions') {
      const rows = db.raw.prepare(
        `SELECT s.session_key, s.claude_session_id, s.cwd, s.model, s.last_active_ts,
         ...
         FROM sessions s WHERE s.chat_id = ? ORDER BY s.last_active_ts DESC LIMIT 10`
      ).all(chatId);
      if (!rows || rows.length === 0) { await sendReply('No sessions found for this chat.'); return true; }
      const lines = rows.map((r, i) => {
        ...
        const preview = r.last_msg ? r.last_msg.slice(0, 60).replace(/\n/g, ' ') : '(empty)';
        return `${i + 1}. \`${r.cwd || '?'}\` — ${agoStr}\n   _${preview}_\n   \`/resume ${r.claude_session_id.slice(0, 8)}\``;
      });
      await sendReply(`📋 **Sessions in this chat:**\n\n${lines.join('\n')}`);
      return true;
    }
