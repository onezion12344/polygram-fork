...
const PATTERNS = {
  ...
  // API rejected request as malformed JSON — broken hex escape, stray
  // control chars in text block, binary leakage. Auto-fixable: sanitize
  // JSONL + retry. Classify as 'hexCorrupt' so the dispatcher can trigger
  // the once-and-for-all auto-recovery path.
  hexCorrupt:       /hex escape|parse the request body as JSON|unterminated string|invalid escape|redacted_thinking|unknown variant/i,
