...
class SdkProcess extends Process {
  ...
  async start(ctx) {
    ...
    // Sanitize session JSONL before --resume: strip broken hex escapes that
    // cause API 400 errors ("unexpected end of hex escape") in deep contexts.
    // Binary content sometimes leaks into text blocks and breaks JSON.
    try {
      const fs = require('fs');
      const raw = fs.readFileSync(this._sessionJsonlPath, 'utf8');
      const clean = raw.replace(BROKEN_HEX_RE, '');
      if (clean !== raw) {
        fs.writeFileSync(this._sessionJsonlPath + '.bak', raw, { mode: 0o600 });
        fs.writeFileSync(this._sessionJsonlPath, clean, { mode: 0o600 });
        this.logger.warn?.(`[${this.label}] sanitized ${raw.length - clean.length} broken hex chars from session JSONL`);
      }
    } catch {}
